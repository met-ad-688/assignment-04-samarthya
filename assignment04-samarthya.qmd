...
- **Model Development**: Implementing and comparing four regression approaches
...
2. **Compare model performance** across Linear Regression, Polynomial Regression, Generalized Linear Models (GLM), and Random Forest
...
"""))
    ...
    # HTML only: Collapsible callout with full column list
    display(Markdown(f"""    
::: {{.content-visible when-format="html"}}
...
if missing_stats:
    ...
    # HTML: Styled callout with table
    display(Markdown(f"""
::: {{.content-visible when-format="html"}}
::: {{.callout-note}}
...
"""))
    ...
    # DOCX/PDF: Simple markdown with proper line breaks
    table_rows_simple = '\n\n'.join([
        f"- {row['Column']}: {row['Missing_Count']:,} ({row['Missing_Percentage']:.1f}%) - {row['Data_Type']}"
        for _, row in missing_df.iterrows()
    ])
...
Initializing four regression models for salary prediction
:::
"""))
...
# 4. Generalized Linear Regression (GLM)
try:
    display(Markdown(f"""
::: {{.callout-tip}}
## Training Generalized Linear Regression
Building GLM with Gaussian family and identity link...
:::
"""))
    
    # GLM with Gaussian family (appropriate for continuous salary data)
    glm = GeneralizedLinearRegression(
        featuresCol="features", 
        labelCol="SALARY_AVG",
        family="gaussian",  # For continuous response variable
        link="identity",    # Identity link: E(Y) = X*β
        maxIter=100,
        regParam=0.1
    )
    ...
    # Cache predictions
    glm_predictions.cache()
    
    glm_rmse = evaluator_rmse.evaluate(glm_predictions)
    glm_r2 = evaluator_r2.evaluate(glm_predictions)
    glm_mae = evaluator_mae.evaluate(glm_predictions)
    
    model_results['Generalized Linear (GLM)'] = {
        'rmse': glm_rmse, 'r2': glm_r2, 'mae': glm_mae,
        'model': glm_model, 'predictions': glm_predictions
    }
...
- **Intercept:** ${glm_model.intercept:,.2f}
- **Number of Coefficients:** {len(glm_model.coefficients)}
:::
"""))
    ...
    display(Markdown(f"""
::: {{.callout-warning}}
## Generalized Linear Regression Training Failed
Error: {str(e)}
:::
"""))
    model_results['Generalized Linear (GLM)'] = None
...
if successful_models:
    try:
        ...
        if all_predictions_list:
            ...
            # HTML: Use styled callouts with tables
            display(Markdown(f"""
::: {{.content-visible when-format="html"}}
::: {{.callout-tip}}
...
"""))
            ...
            for model_name in all_predictions_df['Model'].unique():
                model_df = all_predictions_df[all_predictions_df['Model'] == model_name]
                summary = f"""**{model_name}:**

- Count: {len(model_df):,}
- Avg Actual Salary: ${model_df['Actual_Salary'].mean():,.2f}
- Avg Predicted Salary: ${model_df['Predicted_Salary'].mean():,.2f}
- Avg Absolute Error: ${model_df['Absolute_Error'].mean():,.2f}
"""
...
:::
"""))
            print("\nWARNING: No Predictions to Display")
            print("No successful model predictions available.\n")
...
"""))
    ...
    print("Cannot create predictions DataFrame without trained models.\n")
```
